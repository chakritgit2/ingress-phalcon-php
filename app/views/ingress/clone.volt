...
<form method="post" action="/ingress/store" class="max-w-md space-y-6">
    ...
    <div>
        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300" for="host">Host (<span id="host_label_kind">{{ row.request_type == 'ingress' ? 'โดเมน' : 'IP' }}</span>) *</label>
        <div class="flex gap-2">
            ...
            {% else %}
            <input class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" type="text" id="host" name="host" value="{{ row.host }}" placeholder="192.168.33.31" pattern="^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$" title="สำหรับ NodePort ต้องเป็น IP เช่น 192.168.33.31" required>
            {% endif %}
            <button type="button" id="genHostUuidBtn" title="สุ่ม UUID ใส่ Host" class="{{ row.request_type != 'ingress' ? 'hidden' : '' }} inline-flex shrink-0 cursor-pointer items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
        ...
        <p id="host_help" class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ row.request_type == 'ingress' ? 'ใส่แค่ชื่อโดเมนเปล่าๆ เช่น myapp.advws.com — ห้ามมี http:// หรือ / ต่อท้าย' : 'สำหรับ NodePort ใส่ IP ที่จะใช้เข้าถึง เช่น 192.168.33.31' }}</p>
    </div>
    ...
    <div class="pt-2">
        <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700 cursor-pointer">สร้างคำขอใหม่</button>
    </div>
</form>
...
// Same "pick either field first" behaviour as create.volt/edit.volt: the
// initial load below shows every Deployment across every namespace, and this
...
// so it never clobbers the cloned row's already-carried-over target_port.
...
// Same NodePort-is-an-IP / Ingress-is-a-domain split as create.volt/edit.volt
// — see their comments for why.
var HOST_HINTS = {
    nodeport: {
        kind: 'IP',
...
// The whole point of "clone" is a fresh disposable identity for a host that
// was itself UUID-generated (nodered-<uuid>.advws.org) — carrying the exact
// same host over would make the clone indistinguishable from the expired
// original. Re-roll it once automatically on load. A hand-typed custom
// domain (anything not matching this exact generated pattern) is left as-is
// — the developer chose that hostname on purpose, cloning shouldn't discard it.
var NODERED_UUID_HOST = /^nodered-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.advws\.org$/i;
if ({{ row.request_type == 'ingress' ? 'true' : 'false' }} && NODERED_UUID_HOST.test(hostInput.value)) {
    hostInput.value = 'nodered-' + generateUuidV4() + '.advws.org';
}
...
// conversion the renew dialog on index.volt already does. The expired row's
// schedule_end_minutes only ever meant "duration from whenever the bot
// actually processes this" (see KubernetesTask::processCommandsAction()), so
// pre-filling the picker as now + that same duration is a sensible starting
// point for the clone.
(function () {
