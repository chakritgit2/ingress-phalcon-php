...
<div class="mb-4 flex items-center justify-between rounded-lg border {{ botEnabled ? 'border-green-200 bg-green-50 dark:border-green-500/20 dark:bg-green-500/10' : 'border-red-200 bg-red-50 dark:border-red-500/20 dark:bg-red-500/10' }} px-4 py-3">
    ...
    {% else %}
        <form method="post" action="/statefulsets/toggle-bot">
...
<p class="mb-4 flex items-start gap-1.5 text-xs text-gray-500 dark:text-gray-400">
    ...
    <span>บอททำงานอัตโนมัติทุก 1 นาที: <strong>ประมวลผลคำขอ</strong> (สร้าง/ลบ Service จริงบน Kubernetes ตามคิวที่ค้างอยู่) และ <strong>เก็บกวาดของหมดอายุ</strong> (ลบรายการที่ครบกำหนด "Schedule End" อัตโนมัติ) — ถ้าปิดบอทไว้ คำขอใหม่จะค้างที่สถานะ pending จนกว่าจะเปิดอีกครั้ง</span>
</p>
...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 pb-4 dark:border-gray-800">
    <h1 class="text-xl font-semibold text-gray-900 dark:text-white">StatefulSet Service ที่เปิดอยู่</h1>
    {% if currentUser.isDevops() %}
    <a href="/statefulsets/create" class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700">
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
            <circle cx="12" cy="12" r="9"/>
            <path stroke-linecap="round" d="M12 8v8M8 12h8"/>
        </svg>
        สร้าง StatefulSet Service ใหม่
    </a>
    {% endif %}
</div>

<form method="get" action="/statefulsets" class="mb-4 flex flex-wrap items-end gap-3">
    <div>
    ...
    <table class="w-full min-w-[820px] text-sm">
        ...
        <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
            ...
            <tr class="transition duration-150 hover:bg-gray-50 dark:hover:bg-gray-800/60">
                ...
                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ row.statefulset_name }}</td>
            ...
            {% else %}
            <tr><td class="px-4 py-8 text-center text-gray-500 dark:text-gray-400" colspan="10">
...
{% include "partials/pagination" with ["page": page, "totalPages": totalPages, "totalItems": totalItems, "pageNumbers": pageNumbers, "baseUrl": "/statefulsets", "extraParams": {"namespace": filterNamespace, "developer_name": filterDeveloperName, "status": filterStatus}] %}
    ...
    <form id="renewForm" method="post">
        ...
        <div class="flex items-start gap-3 border-b border-gray-100 px-5 py-4 dark:border-gray-800">
            ...
            <div class="min-w-0 flex-1 pt-0.5">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white">ต่ออายุ Service</h3>
...
(function () {
    ...
    window.openRenewDialog = function (btn) {
        ...
        form.action = '/statefulsets/' + btn.dataset.id + '/renew';
